...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
	...
	$sql= sprintf("SELECT * FROM talonarios WHERE codigocobrador = %s",
	...
	if ($control > 0){
		header ("Location: cobradores.php?error=1");
	} else {

		$deleteSQL = sprintf("DELETE FROM cobrador WHERE codigocobrador=%s",
							GetSQLValueString($_GET['id'], "text"));

		mysql_select_db($database_dbo, $dbo);
		$Result1 = mysql_query($deleteSQL, $dbo) or die(mysql_error());

		$nombre_cob=$_GET['nombre'];
		// ----------------------Auditoria ----------------------//
		$_SQL = sprintf("INSERT INTO logging (usuario, fecha, tipotransaccion, descripcion) VALUES (%s, now(), %s, %s)",
					GetSQLValueString($_SESSION['MM_Username'], "text"),
					GetSQLValueString("Eliminar Cobrador", "text"),
					GetSQLValueString($_GET['id']." ".$nombre_cob, "text"));
