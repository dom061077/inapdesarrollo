...
class ProfesionalController {
    ...
    def update = {
        ...
        if (profesionalInstance) {
			...
			if(fechaIngresoError){
				profesionalInstance.validate()
				profesionalInstance.errors.rejectValue("fechaIngreso","com.medfire.Profesional.fechaIngreso.date.error","Ingrese una fecha correcta, se sugiere una correci�n")
				render(view: "edit", model: [profesionalInstance: profesionalInstance])
				return
			}
			if(fechaNacimientoError){
				profesionalInstance.validate()
				profesionalInstance.errors.rejectValue("fechaNacimiento","com.medfire.Profesional.fechaNacimiento.date.error","Ingrese una fecha correcta, se sugiere una correci�n")
				log.debug "ERROR EN FECHA DE NACIMIENTO SEGUN BANDERA"
				render(view: "edit", model: [profesionalInstance: profesionalInstance])
				return
			}
