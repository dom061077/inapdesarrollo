...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
	...
	$sql= sprintf("SELECT * FROM ingresos WHERE propietario = %s",
	...
	if ($control > 0){
		header ("Location: propietarios.php?error=1");
	} else {
  
		$deleteSQL = sprintf("DELETE FROM propietarios WHERE codigo=%s",
							GetSQLValueString($_GET['id'], "text"));

		mysql_select_db($database_dbo, $dbo);
		$Result1 = mysql_query($deleteSQL, $dbo) or die(mysql_error());
		
		$nombre_prop=$_GET['nombre'];
		// ----------------------Auditoria ----------------------//
		$_SQL = sprintf("INSERT INTO logging (usuario, fecha, tipotransaccion, descripcion) VALUES (%s, now(), %s, %s)",
					GetSQLValueString($_SESSION['MM_Username'], "text"),
					GetSQLValueString("Eliminar Propietario", "text"),
