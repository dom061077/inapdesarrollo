	...
	def save = {
		...
		Carrera.withTransaction{TransactionStatus status ->
			...
			if (carreraInstance.save(flush: true)) {
                ...
                log.debug "ERRORES: "+carreraInstance.errors.allErrors
	...
	def edit = {
		...
		if (!carreraInstance) {
			...
			carreraInstance.niveles.each{
				if(flagcoma){
					nivelesSerialized  = nivelesSerialized +','+ '{"id":'+it.id+',"idNivel":'+it.id+',"descripcion":"'+it.descripcion+'","esprimernivel":"'+it.esprimernivel+'"}'
				}else{
					nivelesSerialized = nivelesSerialized+ '{"id":'+it.id+',"idNivel":'+it.id+',"descripcion":"'+it.descripcion+'","esprimernivel":"'+it.esprimernivel+'"}'
					flagcoma=true
				}
			...
			Carrera.withTransaction(){TransactionStatus status ->
				...
				try{
					if (!carreraInstance.hasErrors() && carreraInstance.save(flush: true)) {
						...
						status.setRollbackOnly()
						flash.message = "Algun registro eliminado en las grillas de registros, niveles o A�os Lectivos esta referenciado en otros datos. Verifique antes de eliminar y vuelva a intentar la modificacion "
						redirect(action:"list")
						return 
				}
			}
